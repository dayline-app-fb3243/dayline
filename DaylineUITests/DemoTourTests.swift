...
final class DemoTourTests: XCTestCase {
    ...
    func testSplashLoopVideo() throws {
        let app = XCUIApplication()
        app.launchArguments = ["-demo", "-onboarding", "-splash.map", "loop", "-splash.pin", "smooth"]
        app.launchEnvironment["TZ"] = Self.morningZone
        app.launch()
    ...
    /// Journal page samples 1-5 plus the current page.
    func testJournalPages() throws {
        for v in ["", "1", "2", "3", "4", "5"] {
            let app = XCUIApplication()
            app.launchArguments = ["-demo", "-journal.page", v]
            app.launch(); pause(1.5)
            tab(app, "Journal"); pause(2.5)
            shot("jp-\(v.isEmpty ? "now" : v)")
            app.terminate()
        }
    }
